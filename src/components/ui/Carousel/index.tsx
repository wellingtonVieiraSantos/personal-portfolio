...
>(({ className, children, ...props }, ref) => {
  const { index, itemCount, setItemCount, itemsPerView, setIndex } =
    useCarousel()
  ...
  useEffect(() => {
    const autoAction = setInterval(() => {
      setIndex(prev => (prev < itemCount - 1 ? prev + 1 : 0))
    }, 3000)

    return () => clearInterval(autoAction)
  }, [itemCount, setIndex])
